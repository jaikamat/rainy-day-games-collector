    ...
    static removeCards(card, filters) {
        for (let prop in filters) {
            if (filters.hasOwnProperty(prop)) {
                if (filters[prop].indexOf(card[prop]) !== -1) {
                    // See if the card prop is in the filter props
                    return false;
                }
            }
        }
...
async function collectSetPages(setHrefs) {
    let setPages = [];
    const browser = await puppeteer.launch();
    let puppetBar = new ProgressBar(
        "Puppeteer working [:bar] :elapsed sec elapsed",
        { total: 50 }
    );

    for (let i = 0; i < setHrefs.length; i++) {
        // Loop over all setcode URI's
        let href = setHrefs[i];
        const page = await browser.newPage();
...
async function getCards() {
    ...
    let collectBar = new ProgressBar(
        "Collecting cards [:bar] :elapsed sec elapsed",
        { total: 50 }
    );
